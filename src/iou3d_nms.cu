...
Iou3dNmsCuda::Iou3dNmsCuda(const float nms_overlap_thresh):
                  nms_overlap_thresh_(nms_overlap_thresh){

}

void Iou3dNmsCuda::nmsNormalLauncher(const float *boxes, unsigned long long * mask, int boxes_num, float nms_overlap_thresh){
    dim3 blocks(DIVUP(boxes_num, THREADS_PER_BLOCK_NMS),
                DIVUP(boxes_num, THREADS_PER_BLOCK_NMS));
    dim3 threads(THREADS_PER_BLOCK_NMS);
...
int Iou3dNmsCuda::DoIou3dNms(const int box_num_pre,const float* res_box,long* host_keep_data)
{
  ...
  GPU_CHECK(cudaMalloc((void**)&mask_data, box_num_pre * col_blocks * sizeof(unsigned long long)));
  nmsLauncher(res_box, mask_data, box_num_pre, nms_overlap_thresh_);

  std::vector<unsigned long long> mask_cpu(box_num_pre * col_blocks);
  GPU_CHECK(cudaMemcpy(&mask_cpu[0], mask_data, box_num_pre * col_blocks * sizeof(unsigned long long),
